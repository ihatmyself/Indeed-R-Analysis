...
analyst_text <- combined_data %>%
  filter(str_detect(category, "data_analyst"))

# Filter for rows where category contains "data_scientist"
scientist_text <- combined_data %>%
  filter(str_detect(category, "data_scientist"))

# Filter for rows where category contains "data_engineer"
engineer_text <- combined_data %>%
  filter(str_detect(category, "data_engineer"))

# Combine job descriptions into a single string
analyst_docs <- paste(analyst_text$job_description, collapse = " ")

scientist_docs <- paste(scientist_text$job_description, collapse = " ")

engineer_docs <- paste(engineer_text$job_description, collapse = " ")
```
